...
function ParticleSingleImage(props){
    ...
    const ParticleImageDisplayer = function(tag_id, canvas_el, params) {
      ...
      */
      pImg.functions.canvas.init = function() {
        ...
        window.addEventListener('resize', pImg.functions.utils.debounce(pImg.functions.canvas.onResize, 500));
      };
      ...
      */
      pImg.functions.particles.SingleImageParticle = function(init_xy, dest_xy) {
        ...
        this.vx = (Math.random() - 0.5) * pImg.particles.movement.speed+3;
        this.vy = (Math.random() - 0.5) * pImg.particles.movement.speed+3;
        this.acc_x = 0;
        this.acc_y = 0;
        this.friction = Math.random() * 0.01 + 0.92;
        this.restlessness = {
          max_displacement: Math.ceil(Math.random() * pImg.particles.movement.restless.value),
          x_jitter: pImg.functions.utils.randIntInRange(-1, 1),
          y_jitter: pImg.functions.utils.randIntInRange(-1, 1),
          on_curr_frame: false
        };
        ...
        this.radius = Math.round((pImg.particles.size.random ? Math.max(Math.random(), 0.25) : 0.5) * pImg.particles.size.value);
      };
      ...
      pImg.functions.particles.createImageParticles = function(pixel_data, at_dest = false) {
        ...
        for (let i = 0; i < pixel_data.width; i += increment) {
          for (let j = 0; j < pixel_data.height; j += increment) {
            if (pixel_data.data[(i + j * pixel_data.width) * 4 + 3] > 100) {
              ...
              pImg.particles.color = "rgb("+pixel_data.data[(j * 4 * pixel_data.width)+ (i * 4)]+","+pixel_data.data[(j * 4 * pixel_data.width)+ (i * 4) +1]+","+pixel_data.data[(j * 4 * pixel_data.width)+ (i * 4) +2]+")";
      ...
      pImg.functions.particles.updateParticles = function() {
        for (let p of pImg.particles.array) {
          if ((pImg.particles.movement.restless.enabled) && (p.restlessness.on_curr_frame)) {
            ...
            p.acc_x = (p.dest_x - p.x) / 450;
            p.acc_y = (p.dest_y - p.y) / 450;
      ...
      pImg.functions.particles.animateParticles = function() {
        ...
        window.requestAnimFrame(pImg.functions.particles.animateParticles);
      };
      ...
      */
      pImg.functions.interactivity.repulseParticle = function(p, args) {
        ...
        const dx_mouse = p.x - pImg.mouse.x,
              ...
              inv_strength = pImg.functions.utils.clamp(400 - args.strength, 10, 500);
      ...
      if(canvas != null){
        ...
        xhr.onreadystatechange = function() {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            window.pImgDom.push(new ParticleImageDisplayer(tag_id, canvas, params));
          } else {
  ...
  return(
        <div
              ...
              id={"particlel-image"} >
          <canvas
                style={{scrollBehavior:"none", position: 'absolute', width: "100%", height: "100%"}} ref={ref} id={"particle-image"} data-params-src={"./params.json"} />
        </div>
  )
  
}
export {ParticleSingleImage}